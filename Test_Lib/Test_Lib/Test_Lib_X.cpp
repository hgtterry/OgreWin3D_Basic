...
void Test_Lib_X::Try(void)
{
	...
	Message((LPTSTR)"Test", (LPTSTR)"TEst");
}
...
void Test_Lib_X::Message(char* pString, char* pString2)
{
	...
	DialogBox(App->hInst, (LPCTSTR)IDD_MESSAGE, App->MainHwnd, (DLGPROC)Proc_Message2);
}

// *************************************************************************
// *        	Message_Proc:- Terry and Hazel Flanigan 2024			   *
// *************************************************************************
LRESULT CALLBACK Test_Lib_X::Proc_Message2(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
	...
	case WM_INITDIALOG:
	{
		...
		SetDlgItemText(hDlg, IDC_STTEXT, App->CL_Test_Lib->Message_Text_Header);
		SetDlgItemText(hDlg, IDC_STMESSAGE, App->CL_Test_Lib->Message_Text_Message);
