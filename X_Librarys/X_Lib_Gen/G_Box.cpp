...
void CX_Box::Box3d_AddPoint(Box3d* b, float px, float py, float pz)
{
...
bool CX_Box::Box3d_Intersection(const Box3d* b1, const Box3d* b2, Box3d* bResult)
{
	bool rslt;
...
void CX_Box::Box3d_Union(const Box3d* b1, const Box3d* b2, Box3d* bResult)
{
	Box3d_Set
	(
		bResult,
...
signed int CX_Box::Box3d_ContainsPoint(const Box3d* b, float px, float py, float pz)
{
...
const T_Vec3* CX_Box::Box3d_GetMin(const Box3d* b)
{
	return &b->Min;
}

const T_Vec3* CX_Box::Box3d_GetMax(const Box3d* b)
{
	return &b->Max;
}
...
float CX_Box::Box3d_GetWidth(const Box3d* b)
{
	return (b->Max.x - b->Min.x + 1);
}

float CX_Box::Box3d_GetHeight(const Box3d* b)
{
	return (b->Max.y - b->Min.y + 1);
}

float CX_Box::Box3d_GetDepth(const Box3d* b)
{
