...
void CL64_Assimp::Create_MeshGroups(const aiScene* pScene)
{
	...
	while (Count < mGroupCount)
	{
		...
		strcpy(App->CL_Mesh->Group[Count]->GroupName, GroupName);

		strcpy(MaterialName, "Material_");
		strcat(MaterialName, GroupNum);
		strcpy(App->CL_Mesh->Group[Count]->MaterialName, MaterialName);

		////---------------

		App->CL_Mesh->Group[Count]->GroupVertCount = 0;
		...
		strcpy(App->CL_Mesh->Group[Count]->MaterialName, mtl->GetName().C_Str());
...
void CL64_Assimp::Get_Group_VertCount(const aiScene* pScene)
{
	...
	while (Count < mGroupCount)
	{
		...
		App->CL_Mesh->Group[Count]->GroupFaceCount = mesh->mNumFaces;
		...
		App->CL_Mesh->Group[Count]->vertex_Data.resize(VC);
		App->CL_Mesh->Group[Count]->Normal_Data.resize(VC);
		App->CL_Mesh->Group[Count]->MapCord_Data.resize(VC);
		App->CL_Mesh->Group[Count]->Face_Data.resize(VC);
...
void CL64_Assimp::StoreMeshData(const aiScene* pScene)
{
	...
	while (GroupCount < mGroupCount)
	{
		...
		App->CL_Mesh->Group[GroupCount]->GroupFaceCount = mesh->mNumFaces;
		App->CL_Mesh->Group[GroupCount]->FaceIndex_Data.resize(mesh->mNumFaces * 3);
		App->CL_Mesh->Group[GroupCount]->Face_Data.resize(mesh->mNumFaces);
		...
		while (t < mesh->mNumFaces)
		{
			...
			while (i < face->mNumIndices) // Triangulated Each face has 3 Indices
			{
				...
				App->CL_Mesh->Group[GroupCount]->vertex_Data[vertexIndex].x = mesh->mVertices[vertexIndex].x;
				App->CL_Mesh->Group[GroupCount]->vertex_Data[vertexIndex].y = mesh->mVertices[vertexIndex].y;
				App->CL_Mesh->Group[GroupCount]->vertex_Data[vertexIndex].z = mesh->mVertices[vertexIndex].z;


				if (mesh->HasNormals())
				{
					App->CL_Mesh->Group[GroupCount]->Normal_Data[vertexIndex].x = mesh->mNormals[vertexIndex].x;
					App->CL_Mesh->Group[GroupCount]->Normal_Data[vertexIndex].y = mesh->mNormals[vertexIndex].y;
					App->CL_Mesh->Group[GroupCount]->Normal_Data[vertexIndex].z = mesh->mNormals[vertexIndex].z;
				}
				...
				{
					App->CL_Mesh->Group[GroupCount]->MapCord_Data[vertexIndex].u = mesh->mTextureCoords[0][vertexIndex].x;
					App->CL_Mesh->Group[GroupCount]->MapCord_Data[vertexIndex].v = 1 - mesh->mTextureCoords[0][vertexIndex].y;
				}
			...
			App->CL_Mesh->Group[GroupCount]->Face_Data[t].a = face->mIndices[0];
			App->CL_Mesh->Group[GroupCount]->Face_Data[t].b = face->mIndices[1];
			App->CL_Mesh->Group[GroupCount]->Face_Data[t].c = face->mIndices[2];

			App->CL_Mesh->Group[GroupCount]->FaceIndex_Data[t].Index = FaceIndexNum;
		...
		App->CL_Mesh->Group[GroupCount]->GroupVertCount = mesh->mNumVertices;
